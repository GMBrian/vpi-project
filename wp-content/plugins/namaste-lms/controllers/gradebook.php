...
class NamasteLMSGradebookController {
	static function manage($in_shortcode = false) {
		global $wpdb;
		...
		if(!empty($_GET['course_id'])) {
			if(!$in_shortcode) do_action('namaste-check-permissions', 'course', $_GET['course_id']);
			...
			$students = $wpdb->get_results($wpdb->prepare("SELECT tU.*, tS.status as namaste_status, tS.grade as course_grade 
		 	...
		 	$students = apply_filters('namaste-gradebook-students', $students, $_GET['course_id']);
	...
	static function view($course_id) {
