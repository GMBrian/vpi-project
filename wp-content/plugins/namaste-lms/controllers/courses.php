...
class NamasteLMSCoursesController {
	...
	static function mass_enroll() {
		...
		if(!empty($_POST['mass_enroll']) and check_admin_referer('namaste_mass_enroll')) {
			...
			foreach($uids as $uid) {
				$_course->enroll($uid, $course->ID, 'enrolled');	
			}
		}
		...
		$page_limit = 20;
		$users = $wpdb->get_results("SELECT SQL_CALC_FOUND_ROWS tU.ID as ID, tU.user_login as user_login, tU.user_email as user_email
				...
				ORDER BY tU.user_login LIMIT $offset, $page_limit"); 		
