...
class NamasteLMSHomeworkController {
	static function submit_solution($in_shortcode = false) {
		...
		if(!$exists) {
			_e('You need to read this lesson before you can submit solutions to assignments.', 'namaste');
			return true; 
		}		
		...
		// now submit
		if(!empty($_POST['ok'])) {
			if(empty($_POST['content']) and empty($_FILES['file']['tmp_name'])) wp_die(__('You cannot submit an empty solution', 'namaste'));			
