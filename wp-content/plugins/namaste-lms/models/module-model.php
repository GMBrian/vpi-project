...
class NamasteLMSModuleModel {
	...
	function select($id = null, $course_id = 0) {
		global $wpdb;
	...
	function complete($module_id, $student_id) {
	  ...
	  $wpdb->query($wpdb->prepare("INSERT INTO ".NAMASTE_HISTORY." SET
			user_id=%d, date=CURDATE(), datetime=NOW(), action='completed_module', value=%s, num_value=%d, module_id=%d",
			$student_id, sprintf(__('Completed module "%s"', 'namaste'), $module->post_title), $module_id, $module_id));
	} // end complete()
